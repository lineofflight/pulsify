...
`ORDER_CHANGE` · Seller Central · listing context
...
function handle(event, context) {
  // No-op by default. Add your custom logic here.
  // Common use cases: order alerts, fulfillment workflows, inventory adjustments.
  return context;
}
```
...
| Path | Type | Example |
| --- | --- | --- |
| `EventTime` | string | `2023-10-03T01:35:06.382Z` |
...
| `NotificationMetadata.PublishTime` | string | `2023-10-03T01:35:07.931Z` |
| `NotificationMetadata.SubscriptionId` | string | `00000000-0000-0000-0000-000000000000` |
| `NotificationType` | string | `ORDER_CHANGE` |
| `NotificationVersion` | string | `1.0` |
| `Payload` | object |  |
| `Payload.OrderChangeNotification` | object |  |
| `Payload.OrderChangeNotification.AmazonOrderId` | string | `123-4567891-4567891` |
| `Payload.OrderChangeNotification.NotificationLevel` | string | `OrderLevel` |
| `Payload.OrderChangeNotification.OrderChangeTrigger` | object |  |
| `Payload.OrderChangeNotification.OrderChangeTrigger.ChangeReason` | string | `Order Status Change` |
| `Payload.OrderChangeNotification.OrderChangeTrigger.TimeOfOrderChange` | string | `2023-10-03T01:35:01.000Z` |
| `Payload.OrderChangeNotification.OrderChangeType` | string | `OrderStatusChange` |
| `Payload.OrderChangeNotification.SellerId` | string | `A1EXAMPLE00001` |
| `Payload.OrderChangeNotification.Summary` | object |  |
| `Payload.OrderChangeNotification.Summary.DestinationPostalCode` | null |  |
| `Payload.OrderChangeNotification.Summary.FulfillmentType` | string | `MFN` |
| `Payload.OrderChangeNotification.Summary.MarketplaceId` | string | `ATVPDKIKX0DER` |
| `Payload.OrderChangeNotification.Summary.OrderItems` | array |  |
| `Payload.OrderChangeNotification.Summary.OrderItems[].OrderItemId` | string | `12345207241` |
| `Payload.OrderChangeNotification.Summary.OrderItems[].Quantity` | number | `15` |
| `Payload.OrderChangeNotification.Summary.OrderItems[].SellerSKU` | string | `EXAMPLE-SKU-001` |
| `Payload.OrderChangeNotification.Summary.OrderItems[].SupplySourceId` | null |  |
| `Payload.OrderChangeNotification.Summary.OrderPrograms` | array |  |
| `Payload.OrderChangeNotification.Summary.OrderStatus` | string | `Unshipped` |
| `Payload.OrderChangeNotification.Summary.OrderType` | string | `StandardOrder` |
| `Payload.OrderChangeNotification.Summary.PurchaseDate` | string | `2023-10-03T01:03:44.106Z` |
| `Payload.OrderChangeNotification.Summary.ShippingPrograms` | array |  |
...
| `listing.deals` | array | yes | Coupons and promotions covering this listing's ASIN whose dates include now, as of dealsReportedAt. Null until both the coupon and promotion reports have synced, so null means unknown, not none. An empty array means both synced and nothing is active. A deal that started and ended between syncs never appears. |
| `listing.dealsReportedAt` | string | yes | ISO 8601 time when both reports last synced (the older of the two types' latest downloads). A deal missing from deals is missing as of this time. It does not say how current each deal's totals are: read the deal's own reportedAt. Null while deals is null. |
| `listing.deals[].data` | object | no | Amazon's report entry with its keys unchanged. A coupon carries its budget and redemption totals; a promotion carries status, type and per-ASIN sales. asins or includedProducts keeps only this listing's ASIN. Totals are cumulative through the day before the last sync. Read status yourself: dates alone do not say whether Amazon cancelled a promotion. |
| `listing.deals[].endsAt` | string | no | ISO 8601, UTC. A deal counts as active while startsAt <= now < endsAt, but Amazon can end one earlier (a budget runs out, or it is cancelled) before the next sync. |
| `listing.deals[].id` | string | no |  |
| `listing.deals[].kind` | string | no |  |
| `listing.deals[].reportedAt` | string | no | ISO 8601, UTC. When this deal's own totals in data were last refreshed from Amazon's report. A deal that started long ago in an older quarter refreshes less often than dealsReportedAt, so read this to judge how current its totals are. |
| `listing.deals[].startsAt` | string | no |  |
